...
/*  Don't let the long code fool you, this can be written shorter, but you don't get the nice interactivity then! See:
http://www.khanacademy.org/cs/peter-de-jong-attractor-simplified/1247996492
in the spinoffs.

Okay, so it's not based on primes, but it's discrete, so that's based on integers!
You take the map:
{x,y}->{sin(a y)-cos(b x),sin(c x)-cos(d y)}
and iterate it over and over again each time drawing a dot at {x,y}, and you end up with this! Pretty cool, right?!

I have a fullscreen one at http://mathandcode.com/dejong/ it runs a bit faster but honestly it's exactly the same thing.
*/
...
var map2=function(v){
    ...
    return {x:sin((a*v.y))-cos((b*v.x)),

        y:sin((c*v.x))-cos((d*v.y))
...
var barA=new TextDraggableBar("A:",10,400-50,180,20,-4,4,1.4,color(0,0,0));

var barB=new TextDraggableBar("B:",10,400-25,180,20,-4,4,-2.3,color(0,0,0));

var barC=new TextDraggableBar("C:",210,400-50,180,20,-4,4,2.4,color(0,0,0));

var barD=new TextDraggableBar("D:",210,400-25,180,20,-4,4,-2.1,color(0,0,0));
...
var draw= function() {
    ...
    if(pointsdrawn<3000000){
        ...
        for(var n=0;n<4000;n++){
            ...
            point(centerx+vec.x*60,centery-vec.y*60);
    ...
    drawBox(20,10,164,45);

    fill(0, 0, 0);

    text("A: "+(""+a).substr(0,5), 25,30);

    text("B: "+(""+b).substr(0,5), 25,50);

    text("C: "+(""+c).substr(0,5), 106,30);

    text("D: "+(""+d).substr(0,5), 106,50);
